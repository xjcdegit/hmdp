...
/*        GeoResults<RedisGeoCommands.GeoLocation<String>> search = stringRedisTemplate.opsForGeo()
                .search(SHOP_GEO_KEY + typeId,
                        ...
                        //includeCoordinates() :带上距离。类似与WithDisTang
        ...
        if(list.size() <= (current - 1)*SystemConstants.DEFAULT_PAGE_SIZE){
            // 没有下一页了，结束
            return Result.ok(Collections.emptyList());
        }
        //4.1 截取需要的部分
        ...
        list.stream().skip((current - 1)*SystemConstants.DEFAULT_PAGE_SIZE).forEach(result ->{
            //获取店铺id
